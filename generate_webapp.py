      ...
      <div class="modal-card">
        ...
        <div class="modal-body" id="adminDashboardBody" style="display: none;">
          ...
          <div style="display: flex; flex-direction: column; gap: 8px;">
            ...
            <button class="btn btn-outline" id="btnClearData" style="color: var(--danger); border-color: #fca5a5;">⚠️ Clear Local Survey Data</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    function setupEventListeners() {{
      ...
      document.getElementById("btnClearData").addEventListener("click", () => {{
        if (confirm("Are you sure you want to permanently clear all survey responses on this device?")) {{
          if (confirm("THIS ACTION CANNOT BE UNDONE. Confirm delete?")) {{
            localStorage.removeItem(LS_SURVEYS);
            refreshAdminStats();
            showToast("🗑️ All survey data cleared.");
          }}
        }}
      }});
